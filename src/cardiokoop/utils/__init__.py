...
def stack_data_with_control(x_data, u_data, num_shifts, len_time):
    """Stack state and control data for Koopman training, with normalization."""

    logger.debug(f"Initial x_data shape: {x_data.shape}")
    logger.debug(f"Initial u_data shape: {u_data.shape}")
    ...
    logger.debug(f"After reshape x_data shape: {x_data.shape}")
    logger.debug(f"After reshape u_data shape: {u_data.shape}")

    # --- Normalize control signal to zero mean and unit variance ---
    u_mean = np.mean(u_data, axis=0)
    u_std = np.std(u_data, axis=0) + 1e-8  # prevent division by zero
    u_data = (u_data - u_mean) / u_std

    logger.debug(
        "Normalized u_data stats: min=%.4f, max=%.4f, mean=%.4f, std=%.4f",
        np.min(u_data), np.max(u_data), np.mean(u_data), np.std(u_data)
    )
    ...
    logger.debug(f"num_traj: {num_traj}, new_len_time: {new_len_time}, nx: {nx}, nu: {nu}")

    x_tensor = np.zeros((num_shifts + 1, num_traj * new_len_time, nx))
    u_tensor = np.zeros((num_shifts,     num_traj * new_len_time, nu))

    for j in range(num_shifts + 1):
        for k in range(num_traj):
            idx = slice(k * new_len_time, (k + 1) * new_len_time)
            x_start = k * len_time + j
            x_tensor[j, idx, :] = x_data[x_start:x_start + new_len_time, :]

    for j in range(num_shifts):
        for k in range(num_traj):
            idx = slice(k * new_len_time, (k + 1) * new_len_time)
            u_start = k * len_time + j
            u_tensor[j, idx, :] = u_data[u_start:u_start + new_len_time, :]

    logger.debug(f"Final x_tensor shape: {x_tensor.shape}")
    logger.debug(f"Final u_tensor shape: {u_tensor.shape}")

    u_flat = u_tensor.reshape(-1, nu)
    logger.debug(
        "u_tensor stats: min=%.4f, max=%.4f, mean=%.4f, isnan=%d, isinf=%d",
        np.min(u_flat), np.max(u_flat), np.mean(u_flat), np.isnan(u_flat).sum(), np.isinf(u_flat).sum()
    )
...
def stack_data(data, num_shifts, len_time):
    """Stack data from a 2D array into a 3D array, with debug messages."""
    ...
    logger.debug(f"After reshape data shape: {data.shape}")
    logger.debug(f"num_traj: {num_traj}, new_len_time: {new_len_time}, n: {n}")

    data_tensor = np.zeros([num_shifts + 1, num_traj * new_len_time, n])

    for j in range(num_shifts + 1):
        for count in range(num_traj):
            idx = slice(count * new_len_time, (count + 1) * new_len_time)
            data_tensor[j, idx, :] = data[count * len_time + j : count * len_time + j + new_len_time, :]

    logger.debug(f"Final data_tensor shape: {data_tensor.shape}")
    logger.debug(
        "data_tensor stats: min=%.4f, max=%.4f, mean=%.4f, isnan=%d, isinf=%d",
        data_tensor.min(), data_tensor.max(), data_tensor.mean(), np.isnan(data_tensor).sum(), np.isinf(data_tensor).sum()
    )
