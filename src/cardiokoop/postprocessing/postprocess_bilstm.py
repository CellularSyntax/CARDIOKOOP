#!/usr/bin/env python
"""
Fast post-processing for the **Small-BiLSTM** baseline
=====================================================

Outputs the same metric set – and the same results-pickle structure –
as the Koopman / GRU / LSTM post-processors.

  %RMSE variants
  --------------
    • pct_traj_flat   – flattened (old style) per sequence
    • pct_traj_ps     – per-signal %RMSE averaged inside each sequence
    • pct_global_flat – one %RMSE for the whole validation split
"""
# ───────────────────────── imports / CLI ─────────────────────────
...
from cardiokoop.training.train_bilstm import SmallBiLSTM, autoregressive_rollout

def parse_args():
    p = argparse.ArgumentParser(description="Post-process Small-BiLSTM run")
    p.add_argument("--exp-folder",  default="../results", help="results directory")
    p.add_argument("--model-path", help="explicit *.ckpt")
    p.add_argument("--error-csv",  help="explicit *_error.csv")
    p.add_argument("--data-folder", default="data",       help="root with normalised csv")
...
# ───────────────── locate latest ckpt / csv if not given ────────────
...
model    = SmallBiLSTM(input_dim, params['hidden'], params['dropout'], n_signals).to(device)
# load checkpoint and eval model
...
cum_pct=[]   # Koopman-style cumulative %RMSE curves
...
for i, (x_n, x_org) in enumerate(zip(X_norm, X_true)):
    ...
    # limits-of-agreement (per seq, per signal)
    loa_low_k  = bias_k - 1.96*sigma_k
    loa_high_k = bias_k + 1.96*sigma_k

    # flattened %RMSE for this sequence
    ...
    pct_flat.append(pct_f);         pct_ps.append(pct_k.mean())
...
if __name__ == "__main__":
    ...
    out_path = os.path.join(exp_folder, "bilstm_postprocessing_results.pkl")
