#!/usr/bin/env python3
"""
Export every number that appears in the
...
* The committed ``results/koopman/koopman_postprocessing_results.pkl`` is the
  *validation*-split (val1) evaluation, whereas every baseline
  pickle is test1.  This script writes a test1 Koopman pickle
  (``results/tables/koopman_test1_postprocessing_results.pkl``) with the
  same keys as the baseline pickles so that every manuscript number has a
  committed file of origin.
* The MLP and AR(20) rows of Table 4 are produced with the shared functions
  (``scripts/experiments/build_tables_figures.py``: ``mlp_pred`` with the committed
...
Metric conventions (identical to ``scripts/experiments/_common.py``)
------------------------------------------------------------------
...
Outputs (results/tables/) — .md/.json/.tsv only (``*.csv`` is Git-LFS filtered)
----------------------------------------------------------------------------------
...
Usage
-----
  python scripts/tables/export_manuscript_tables.py            # float64 Koopman rollout (default)
  python scripts/tables/export_manuscript_tables.py --dtype float32
  python scripts/tables/export_manuscript_tables.py --out-dir out/   # write elsewhere (Docker/CI check)
...
to these differences (see docs/REPRODUCIBILITY.md).
"""
import os
import sys
import json
...
REV2 = os.path.join(os.path.dirname(HERE), "experiments")
...
REV3_DIR = os.path.join(C.RESULT_DIR, "tables")
...
# simulation (manuscript Table 3, measured on the training machine (GPU) when the
...
def mlp_recompute_check(Xn, Xte, sig_mean, sig_std, pred_frozen, frozen_meta):
    ...
    out["note"] = ("The MLP baseline diverges (clip ±20, %RMSE > 100 %), so its 1499-step float32 rollout amplifies "
                   ...
                   "current machine (gated at 2 % relative by scripts/tables/compare_results.py).")
    return out
...
def main():
    ...
    ap.add_argument("--out-dir", default=None,
                    help="write all outputs to this directory instead of results/tables "
    ...
    ap.add_argument("--recompute-mlp", action="store_true",
                    help="re-roll the MLP baseline (float32) instead of loading the frozen test1 predictions "
                         "results/mlp/mlp_postprocessing_results.pkl (platform-sensitive; see docs/REPRODUCIBILITY.md)")
    args = ap.parse_args()
    if args.out_dir:
        global REV3_DIR
    ...
    # published per-trajectory inference times (Table 3, measured on the training
    # machine, GPU) are re-used exactly as in scripts/experiments/build_tables_figures.py
    ...
    # mlp_recompute_check.json (see docs/REPRODUCIBILITY.md, "Container verification").
    mlp_check = None
    t0 = time.time()
    ...
    write_table("table3_overall", hdr3, rows_md, rows_js,
                ...
                "Inference times / speed-ups are the published measurements on the training machine, GPU (see build_tables_figures.py).")
    ...
    write_table("table5_noise", hdr5, rows_md, rows_js,
                ...
                "experiments/noise_robustness_linear.json.")
