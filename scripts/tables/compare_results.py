#!/usr/bin/env python3
"""
Compare a fresh run of ``scripts/tables/export_manuscript_tables.py`` against the
committed ``results/tables/`` at **manuscript rounding**.
...
is unchanged:
    ...
    r2_conventions.json
              ...
              global pooled R² ................. 3 decimals (as quoted in docs/REPRODUCIBILITY.md)
...
Usage
-----
  python scripts/tables/compare_results.py --committed results/tables --fresh out/
  python scripts/tables/compare_results.py ... --report out/compare_report.md --all
  python scripts/tables/compare_results.py ... --tolerance-json my_tolerances.json
...
def main():
    ...
    if args.report:
        with open(args.report, "w") as f:
            f.write("# Reproduction check — committed `results/tables` vs. fresh run\n\n")
