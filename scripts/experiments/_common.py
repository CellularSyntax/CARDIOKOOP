#!/usr/bin/env python3
"""
Shared utilities for the baseline, ablation and robustness experiments in scripts/experiments/.

This module centralises everything the experiment scripts need so that every new
...
* Split:  the manuscript baselines (LSTM/GRU/BiLSTM/MLP/AR20) and the noise
  ...
  row in ``results/koopman/koopman_postprocessing_results.pkl`` was generated on ``val1``; the two splits are within
...
REV2_DIR    = os.path.join(RESULT_DIR, "experiments")
...
# RNN representative (GRU -- the strongest of GRU/LSTM/BiLSTM), BOTH direct
# linear baselines (DLinear/NLinear), the naive MLP anchor, and the
